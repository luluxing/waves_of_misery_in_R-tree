...
public abstract class Buffer<O, I, E> {
	...
	protected class Slot {
		...
		protected void update(E object, Function<Object, ?> flush) {
			// LX: update the stored bytes
			Tree.Node old = (Tree.Node) this.object;
//			System.out.println("old " + old.number() + ", new " + ((Tree.Node) object).number());
			if (old.level() == 0) {
				Buffer.this.storedLeafEntry -= old.number();
				Buffer.this.storedLeaf--;
			} else {
				Buffer.this.storedIndexEntry -= old.number();
				Buffer.this.storedIndex--;
			}

			this.object = object;
			if (((Tree.Node) object).level == 0) {
				Buffer.this.storedLeafEntry += ((Tree.Node) object).number();
				Buffer.this.storedLeaf++;
			}  else {
				Buffer.this.storedIndexEntry += ((Tree.Node) object).number();
				Buffer.this.storedIndex++;
			}
		...
		protected void insert(O owner, I id, E object) {
			...
			members.put(id, this);
			size++;
//			System.out.println("Insert " + ((Tree.Node) object).number() + ", now size is " + size +
//					", leaf " + storedLeaf + ", index " + storedIndex);
		}
		...
		protected void remove() {
			if (index < size) {
				...
				Tree.Node old = (Tree.Node) object;
				...
				object = null;
//				System.out.println("remove " + old.number() + ", now size is " + size +
//						", leaf " + storedLeaf + ", index " + storedIndex);
			}
		}
	...
	/**
	 * Added by LX to record the number of slot removed from buffer
	 */
	public int displaceNum = 0;

	/**
	 * Added by LX
	 */
	public long storedLeafEntry;
	public long storedIndexEntry;
	public int storedLeaf;
	public long storedIndex;

	/**
	 * Added by LX to obtain the current utilization of the buffer
	 */
	public double getBufferUtil() {
	 ...
	 * Added by LX to obtain the current utilization of the buffer
	 */
	public void countBufferEntry() {
		storedLeafEntry = 0;
		storedIndexEntry = 0;
		for (int i = 0; i < size; i++) {
			Object obj = slots.get(i).object;
			int num = ((Tree.Node) obj).number();
			if (((Tree.Node) obj).level() == 0) {
				storedLeafEntry += num;
			} else {
				storedIndexEntry += num;
			}
		}
	}
...
//		System.out.printf("Find %d (hash %d) in buffer takes %d ns\n", id, hash(id), (end - start));
		
		if (slot == null) {
			if (fixedSlots == slots.size()) {
				System.out.println("Fixed slots " + fixedSlots);
				throw new IllegalStateException("Buffer overflow. Too many slots fixed.");
			}
