...
public class DoublePointRectangle implements Rectangle {
	 ...
	 * Changed from protected to public by LX
	 */
	...
	*/
	 ...
	 * @param dp
	 */
	public void union(DoublePoint dp) {
		for (int i = leftCorner.length; --i >= 0;) {
			if (leftCorner[i] > dp.getValue(i))
				leftCorner[i] = dp.getValue(i);
			if (rightCorner[i] < dp.getValue(i))
				rightCorner[i] = dp.getValue(i);
		}
	}
