...
public class STRBulkLoader<T> extends AbstractIterativeRtreeBulkloader<T>{
	...
	public void buildRTree(Iterator rectangles) throws IOException{
		...
		// Before init, record some metadata first
		long leafNodeNum = Math.round(rtree.leafNodeNum);
		double totalCap = rtree.totalLeafNodeCapacity;
		double numDataInLeaf = rtree.numOfDataInLeafNodes;
		...
		// Added by LX
		rtree.leafNodeNum = leafNodeNum;
		rtree.totalLeafNodeCapacity = totalCap;
		rtree.numOfDataInLeafNodes = numDataInLeaf;
	}
...
//				DoublePointRectangle rec = null;
				...
				if(level != 0 ){
					...
					// create index entry 
					IndexEntry indexEntry = (IndexEntry) rtree.createIndexEntry(level);
					((ORTree.IndexEntry)indexEntry.initialize(mapEntry.getKey())).initialize((DoublePointRectangle) rec);
					entries.add(indexEntry);
				}else{
					rec = sorter.next();
					entries.add(rec);
					rtree.numOfDataInLeafNodes++;
				}
			}
			if (level == 0) {
				rtree.leafNodeNum++;
				rtree.totalLeafNodeCapacity += rtree.singleLeafNodeCapacity;
			}
	...
	protected Comparator getDimensionComparator(final int dim, final int level){
		return  new Comparator() {
			...
			public int compare(Object  o1, Object o2) {
				...
				DoublePoint first;
				DoublePoint second;
				if (o1 instanceof DoublePoint && o2 instanceof DoublePoint) {
					// To accommodate DoublePoint used by LX
					first = (DoublePoint) o1;
					second = (DoublePoint) o2;
				} else {
					...
					first = rec1.getCenter();
					second = rec2.getCenter();
				}
