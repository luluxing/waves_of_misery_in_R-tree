...
//			return new SplitInfo(path).initialize(nodesMBRs[1]);
			Tree.Node.SplitInfo spi = new SplitInfo(path).initialize(nodesMBRs[1]);
			spi.splitOverlap = nodesMBRs[1].overlap(nodesMBRs[0]);// Added by LX
			spi.splitOverlapRatio = spi.splitOverlap / (Math.min(nodesMBRs[1].area(), nodesMBRs[0].area()));
			spi.splitOverlapRatio2 = spi.splitOverlap / (Math.max(nodesMBRs[1].area(), nodesMBRs[0].area()));
			spi.splitRatio = (Math.min(nodesMBRs[1].area(), nodesMBRs[0].area())) / (Math.max(nodesMBRs[1].area(), nodesMBRs[0].area()));
			spi.areaDiff = Math.abs(nodesMBRs[1].area() - nodesMBRs[0].area());
			return spi;
		}
	}
}
