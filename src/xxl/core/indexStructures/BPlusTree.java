...
public class BPlusTree extends Tree {
   ...
   protected int B_LeafNode;
    ...
    /** LX: Deferred split or ordinary split */
    protected int siblingNum;
    ...
    public BPlusTree(int blockSize, double minCapacityRatio) {
        this(blockSize,  minCapacityRatio, false , 1);
    }

    /**
     * Added by LX: enable deferred split
     * @param blockSize
     * @param minCapacityRatio
     */
    public BPlusTree(int blockSize, double minCapacityRatio, int siblingNum){
        this(blockSize, minCapacityRatio, false, siblingNum);
    }
    ...
    public BPlusTree(int blockSize, boolean allowDuplicate){
    	this(blockSize, 0.5f, allowDuplicate, 1);
    }
     ...
     *            the minimal capacity ratio of the tree's nodes
     */
    public BPlusTree(int blockSize, double minCapacityRatio, boolean allowDuplicate, int siblingNum) {
        super();
    ...
    initialize(Function getKey,
        ...
        // Added by LX
        this.singleLeafNodeCapacity = this.B_LeafNode;
        this.singleIndexNodeCapacity = this.B_IndexNode;
    ...
    // Add by LX
    protected HilbertRTree.ORSeparator ORseparator(Object entry) {
        return (HilbertRTree.ORSeparator) getDescriptor.invoke(entry);
    }
    /**
...
//              // test code
   ...
   * 			when the tree is in duplicate mode 
    ...
    public class IndexEntry extends Tree.IndexEntry {
        ...
        /** Added by LX */
        public HilbertRTree.ORSeparator ORseparator() {
            return (HilbertRTree.ORSeparator) separator;
        }
        /*
         * (non-Javadoc)
...
//            }
        ...
        protected Tree.IndexEntry chooseSubtree(Descriptor descriptor,
            ...
            // LX
            // If there is only one entry in this node, then it cannot perform redistribution
            if (this.number() == 1) {
                siblingPath.push(new MapEntry(true, null)); // put a dummy entry
            } else {
                Object entry = entries.get(minIndex == this.number() - 1 ? minIndex - 1 : minIndex + 1);
                boolean isRight = minIndex == this.number() - 1 ? false : true;
                siblingPath.push(new MapEntry(isRight, entry));
            }
            return  subTreeEntry;
        }
        /**
...
//            System.out.println(index + ", " + entries.size() + ", " + entry + "," + separator(entry));
        }
        /** 
    ...
    protected class QueryCursor extends xxl.core.indexStructures.QueryCursor {
         ...
         * Added by LX
         */
        public int[] getNodeAccess() { int[] a = {0, 0}; return a; };
    ...
    public class NodeConverter extends Converter {
       /**
        ...
        protected int headerSize() {
            return 2 * IntegerConverter.SIZE
                    + LongConverter.SIZE // for nodeID by LX
//                    + BooleanConverter.SIZE + BPlusTree.this.container().getIdSize() // for prevNeighbor by LX
                    + BooleanConverter.SIZE
