...
//			return new SplitInfo(path).initialize(computeDescriptor(this.entries));
			...
			Rectangle oldDescriptor = (Rectangle) computeDescriptor(node.entries);
			((IndexEntry)indexEntry(path)).descriptor = oldDescriptor;
			Rectangle newDescriptor = (Rectangle) computeDescriptor(this.entries);
			Tree.Node.SplitInfo spi = new SplitInfo(path).initialize(newDescriptor);
			spi.splitOverlap = newDescriptor.overlap(oldDescriptor);// Added by LX
			spi.splitOverlapRatio = spi.splitOverlap / (Math.min(newDescriptor.area(), oldDescriptor.area()));
			spi.splitOverlapRatio2 = spi.splitOverlap / (Math.max(newDescriptor.area(), oldDescriptor.area()));
			spi.splitRatio = Math.min(newDescriptor.area(), oldDescriptor.area()) / Math.max(newDescriptor.area(), oldDescriptor.area());
			spi.areaDiff = Math.abs(newDescriptor.area() - oldDescriptor.area());
			return spi;
		}
	}
}
