...
public class RevisedRTree extends RTree{
	...
	*/
	...
	/** Modified by LX to keep the number of accessed leaf node */
	public Cursor query (final Descriptor queryDescriptor, final int targetLevel, final int[] nodeAccess){
		//
		final Iterator [] iterators = new Iterator[height()+1];
		Arrays.fill(iterators, EmptyCursor.DEFAULT_INSTANCE);
		if (height()>0 && queryDescriptor.overlaps(rootDescriptor()))
...
//			 chooseSubTree aktulisiert den Descriptor von IndexEntry von Uebergelaufenen Knoten	
			...
			int minEntriesTemp = node.splitMinNumber();
			int maxEntriesTemp = node.splitMaxNumber();
			if (RevisedRTree.this.regularElectiveSplit) {
				// need to add the entries from the overflow nodes
				if (level() == 0 && RevisedRTree.this.pseudoOverflowNodes.containsKey(node.nodeId)) {
					List overflowPages = (List) RevisedRTree.this.pseudoOverflowNodes.get(node.nodeId);
					for (Object list: overflowPages) {
						for (Object entry: (List) list) {
							node.entries.add(entry);
						}
						RevisedRTree.this.totalOverflowEntryNum -= ((List<?>) list).size();
					}
					RevisedRTree.this.totalOverflowPageNum -= overflowPages.size();
					RevisedRTree.this.pseudoOverflowNodes.remove(node.nodeId);
//					hasOverflowPage = true;
				}
				// need to handle min/max number
				if (node.number() < minEntriesTemp) {
					minEntriesTemp = node.number() / 2;
					maxEntriesTemp = node.number() - minEntriesTemp;
//					System.out.println(minEntriesTemp + "," + maxEntriesTemp);
				}
			}
			...
			// LX: unequal random split
			if (unequalRandomSplit) {
				Random rand = new Random();
				List distributionList = new ArrayList();
				while (itListDistr.hasNext()) {
					Object tmp = itListDistr.next();
				...
				goalDistribution = (RevisedRTree.Node.Distribution) distributionList.get(idx);
			} else {
				if (node.level()> 0){ //inner Nodes
					...
					final Function<Distribution, Double> computeCost = createWeightFunktion(minEntries,
							node.number(), pMax, middlePointN,
				...
				goalDistribution = (Distribution)Cursors.minima(iteratorWeightedDist,
			...
			node.entries.clear();
			node.entries.addAll(goalDistribution.entries(false));
			entries.addAll(goalDistribution.entries(true));

			// LX: RES remedy
			// need to assign a node id to the newly node
			if (regularElectiveSplit && node.level() == 0) {
				long nid = node.nodeId;
				long nnid = leafNodeNum + 1;
				nodeId = nnid;
				Iterator it = node.entries();
				if (it.hasNext()) {
					Object obj = it.next();
					if (obj != null)
						leafNodeToRange.put(nid, getDescriptor.invoke(obj));
				}
				it = entries();
				if (it.hasNext()) {
					Object obj = it.next();
					if (obj != null)
						leafNodeToRange.put(nodeId, getDescriptor.invoke(obj));
				}
			}

			// update the descriptor of the old index entry
			((IndexEntry)indexEntry(path)).descriptor = goalDistribution.descriptor(false);
			//setMiddlePoint
			node.setMiddlePoint(goalDistribution.descriptor(false));
			this.setMiddlePoint(goalDistribution.descriptor(true));
			return new SplitInfo(path).initialize(goalDistribution);
		}
		/**
		...
		protected Iterator<List<Distribution>> getAllDistributions(final Node node){
			...
			int maxEntriesTemp = node.splitMaxNumber();
			// LX: spread split
//			System.out.println(minEntriesTemp + "," + maxEntriesTemp + ", " + node.number());
