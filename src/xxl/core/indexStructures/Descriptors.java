...
public abstract class Descriptors {
	...
	/**
	 * Added by LX to compute the overlap for split in LinearRtree
	 * @param r1
	 * @param r2
	 * @return
	 */
	public static double overlap(Rectangle r1, Rectangle r2) {
		DoublePointRectangle rect = (DoublePointRectangle) r2;
		double overlap = 1.0;
		for (int i = 2;
			 --i >= 0
					 && (overlap
					 *=
					 Math.min(
							 ((DoublePointRectangle)r1).rightCorner[i],
							 rect.rightCorner[i])
							 - Math.max(
							 ((DoublePointRectangle)r1).leftCorner[i],
							 rect.leftCorner[i]))
					 > 0;
		)
			;
