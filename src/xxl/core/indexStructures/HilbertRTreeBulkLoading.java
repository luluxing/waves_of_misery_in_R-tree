...
public class HilbertRTreeBulkLoading extends BPlusTreeBulkLoading {
     ...
     */
     public HilbertRTreeBulkLoading(HilbertRTree tree, Iterator objects){
		 this(tree,  objects, tree.getContainer, tree.overflows , HilbertRTreeBulkLoading.ASCENDING_BULKLOAD);
     }
	 ...
	 * @param initCap
	 */
	public HilbertRTreeBulkLoading(HilbertRTree tree, Iterator objects, double initCap){
		super(tree, objects, tree.getContainer, tree.overflows, HilbertRTreeBulkLoading.ASCENDING_BULKLOAD, initCap);
		...
		((HilbertRTree.IndexEntry) btree.rootEntry).hilbertEntryMBR = rootsMBR; // LX
	}
	/**
	 * Added by LX
	 * @param tree
	 * @param objects
	 */
	public HilbertRTreeBulkLoading(HilbertRTree tree, Iterator objects, int seed, int indexSize) throws FileNotFoundException {
		this( tree, objects, tree.getContainer, HilbertRTreeBulkLoading.ASCENDING_BULKLOAD, seed, indexSize);
	}

	/**
	 * Added by LX
	 */
	public HilbertRTreeBulkLoading(HilbertRTree tree, Iterator objects, double initCap, int seed, boolean isLinear) throws FileNotFoundException {
		super(tree, objects, tree.getContainer, tree.overflows, HilbertRTreeBulkLoading.ASCENDING_BULKLOAD, initCap, seed, isLinear);
		Node node = (Node)btree.rootEntry.get();
		Rectangle rootsMBR =  ((HilbertRTree)btree).computeMBR(node.entries());
		((HilbertRTree.ORKeyRange)btree.rootDescriptor).entryMBR = rootsMBR;
	}
	 ...
	 * @param order
	 */
	public HilbertRTreeBulkLoading(HilbertRTree tree, Iterator objects, Function<BPlusTree.Node,Container> determineContainer, boolean order, int seed, int indexSize) throws FileNotFoundException {
		this(tree,  objects,  determineContainer, tree.overflows , order, seed, indexSize);
	}
	 ...
	 * @param indexSize
	 */
	public HilbertRTreeBulkLoading(HilbertRTree tree, Iterator objects, Function<Node, Container> determineContainer, Predicate overflows, boolean order, int seed, int indexSize) throws FileNotFoundException {
		super(tree, objects, determineContainer, overflows, order, seed, indexSize);
		Node node = (Node)btree.rootEntry.get();
		Rectangle rootsMBR =  ((HilbertRTree)btree).computeMBR(node.entries());
		((HilbertRTree.ORKeyRange)btree.rootDescriptor).entryMBR = rootsMBR;
	}
	 ...
	 */ 
	   ...
	   protected void adjustFlankPath(Separator newSep, int level, Rectangle flankNodeMBR, Rectangle siblingMBR){
		   ...
		   if (level < bufferPath.size()){
			   ...
			   if (level > 0  && newSep != null) {//below root node and have created new Separator with redistribute 
	    			if (descending){ // left flank	
						...
						((HilbertRTree.IndexEntry)flankNode.getFirst()).hilbertEntryMBR = flankNodeMBR; // LX
	    					if (flankNode.number() > 1) {
								((HilbertRTree.ORSeparator) ((IndexEntry) flankNode.getEntry(secondIndex)).separator).updateMBR(siblingMBR);
								((HilbertRTree.IndexEntry) flankNode.getEntry(secondIndex)).hilbertEntryMBR = siblingMBR;
							}
	    					else {
								((HilbertRTree.ORSeparator) ((IndexEntry) sibling.getFirst()).separator).updateMBR(siblingMBR);
								((HilbertRTree.IndexEntry) sibling.getFirst()).hilbertEntryMBR = siblingMBR;
							}
	    			}else{ // right flank
	        				if (flankNode.number() > 1){//more than one item in right node, therefore update
								...
								((HilbertRTree.IndexEntry)flankNode.getEntry(flankNode.number()-2)).hilbertEntryMBR = siblingMBR;
	        				}
	        				else{
								...
								((HilbertRTree.IndexEntry)sibling.getLast()).hilbertEntryMBR = siblingMBR;
	        				}
	        				((HilbertRTree.ORSeparator)((IndexEntry)flankNode.getLast()).separator).updateMBR(flankNodeMBR);
						((HilbertRTree.IndexEntry)flankNode.getLast()).hilbertEntryMBR = flankNodeMBR;
	    ...
	    protected  BPlusTree.IndexEntry saveBulk (Object id, BPlusTree.Node node, boolean isDuplicateEnabled){
			...
			// LX
			HilbertRTree.IndexEntry indexEntry = (HilbertRTree.IndexEntry) ((HilbertRTree)btree).createIndexEntry(node.level+1);
			indexEntry = (HilbertRTree.IndexEntry) ((IndexEntry) indexEntry).initialize(sep);
			indexEntry = (HilbertRTree.IndexEntry) indexEntry.initialize(container, id);
//					(((BPlusTree)btree).createIndexEntry(node.level+1)).initialize(sep).initialize(container, id);
			indexEntry.hilbertEntryMBR = nodesMBR;
			indexEntry.ORDescriptor = (HilbertRTree.ORSeparator) sep;
			return indexEntry;
	    }
}
