...
//		boolean reopen = (new File(args[0]+".ctr")).canRead();
		boolean reopen = (new File("bulk.ctr")).canRead();
		...
		if (!reopen) {
			...
			fileContainer = new BlockFileContainer("bulk.ctr", blockSize);
		}
		else {
...
//		}
		...
		// LX bulk-loading
		System.out.println("Begin bulk loading of R tree");
		...
		Cursor cursor = new FileInputCursor<>(objectConverter, new File(pointsFile));

		Iterator<DoublePoint> sortedIterator = new MergeSorter<DoublePoint>(cursor, WavesMisery.COMPARE_HILBERT, dataSize, 4*4096, 4*4096);
		new SortBasedBulkLoading(rtree, sortedIterator, rtree.determineContainer);
		System.out.println("Done bulk loading of R tree");
		/*********************************************************************/
		/*                             RANGE QUERY                           */
		/*********************************************************************/
