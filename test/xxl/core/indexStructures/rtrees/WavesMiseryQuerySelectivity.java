 ...
 */
public class WavesMiseryQuerySelectivity {
...
//			return ((Rectangle)(((KPE)o1).getData())).compareTo(((KPE)o2).getData());
     ...
     */
    public static Comparator getHilbertDistanceBasedComparator (DoublePoint queryObject) {
    ...
    public static Comparator<Object> getORtreeDistanceBasedComparator (DoublePoint queryObject) {
        final Rectangle query = new DoublePointRectangle(queryObject, queryObject);
...
//            container.flush();
            ...
            // Generate queries based on a fixed selectivity
            // knn query
            if (reader.m_performknnQuery) {
                DataOutputStream dataOut = new DataOutputStream(
                        new FileOutputStream(reader.m_outputDir + "query_" + reader.m_queryArea + "_" + j));
                double querySelectivity = reader.m_queryArea;
                int neighbourSize = (int) ((reader.m_primaryIndexSize + reader.m_batchSize * (j + 1)) * querySelectivity);
                double[] focal = new double[dimension];
                int queryNumber = 0, triedNumber = 0;
                while (queryNumber < reader.m_queryNumber) {
                    triedNumber++;
                    if (triedNumber > 50 * reader.m_queryNumber)
                        break;
                    // should be within the universe
                    focal[0] = genQuery.nextDouble() * (reader.m_xMax-reader.m_xMin) + reader.m_xMin;
                    focal[1] = genQuery.nextDouble() * (reader.m_yMax-reader.m_yMin) + reader.m_yMin;
                    DoublePoint queryObject = new DoublePoint(focal);
                    Iterator nnIterator;
                    if (reader.m_treeName.startsWith("hilbert"))
                        nnIterator = ((HilbertRTree)tree).
                                query(new DynamicHeap(getHilbertDistanceBasedComparator(queryObject)), 0);
                    else
                        nnIterator = ((RTree)tree).
                                query(new DynamicHeap(getORtreeDistanceBasedComparator(queryObject)), 0);
                    Cursor knnResults = new Taker(nnIterator, neighbourSize);
                    if (!knnResults.hasNext())
                        continue;
                    DoublePoint next = (DoublePoint)((Candidate)knnResults.next()).entry();
                    if (!knnResults.hasNext())
                        continue;
                    DoublePoint next2 = (DoublePoint)((Candidate)knnResults.next()).entry();
                    double xmin = next.getValue(0) < next2.getValue(0) ? next.getValue(0) : next2.getValue(0);
                    double xmax = next.getValue(0) > next2.getValue(0) ? next.getValue(0) : next2.getValue(0);
                    double ymin = next.getValue(1) < next2.getValue(1) ? next.getValue(1) : next2.getValue(1);
                    double ymax = next.getValue(1) > next2.getValue(1) ? next.getValue(1) : next2.getValue(1);
                    DoublePoint left = new DoublePoint(new double[]{xmin, ymin});
                    DoublePoint right = new DoublePoint(new double[]{xmax, ymax});
                    DoublePointRectangle query = new DoublePointRectangle(left, right);
                    while (knnResults.hasNext()) {
                        next = (DoublePoint)((Candidate)knnResults.next()).entry();
                        query.union(next);
                    }
                    knnResults.close();
                    ...
                    Cursor rangeResults;
                    if (reader.m_treeName.startsWith("hilbert"))
                        rangeResults = ((HilbertRTree)tree).queryOR(query, 0, nodeAccess);
                    else
                        rangeResults = ((RTree)tree).query(query, 0, nodeAccess);
                    int resultSize = Cursors.count(rangeResults);
                    rangeResults.close();
                    if (resultSize <= 2 * neighbourSize) {
                        query.write(dataOut);
                        queryNumber++;
                    }
                }
            ...
            String queryFileName = reader.m_queryFile + "query_" + reader.m_queryArea + "_" + j;
            Cursor queryCursor = new FileInputCursor<DoublePointRectangle>(rectangleConverter, new File(queryFileName));
            ...
            int querySize = reader.m_queryNumber < queries.size() ? reader.m_queryNumber : queries.size();
